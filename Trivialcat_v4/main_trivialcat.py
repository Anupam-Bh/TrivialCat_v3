...
filepath = os.path.abspath(sys.argv[1]+'/scf/')   ### Read the folder where WAVECARS are located. mp 
#print(exe_model_path,filepath,sys.argv)
...
prediction_bandwidth = 2    ## Flat bands are checked within [-1,1] ev
n_samp_pts =  21
...
destructive_interference_threshold = 0.20   ##  sum(wavefunction)/sum(abs(wavefunction)) <20% identifies as destructive interference
...
print(f'Emin: {Emin}, Emax: {Emax}, Sublattice element: {sublattice_elem}, E_fermi = {fermi}, segments={segments}')
